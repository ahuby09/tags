	        ...
	        <div class="top-grid-cat col-xs-6">
				<a href="<?php echo esc_url( $category_link ); ?>"> 
					...
					<div class="top-grid-heading">
						...
						<p>
							<?php if ( $desc ) { echo esc_html( substr( $desc, 0, 30 ) ), '...'; } ?>
						</p>
					</div>
				</a> 
			</div>
					...
					while ( $loop->have_posts() ) : $loop->the_post();
						...
						<li class="product-cats col-xs-6">    
							<a href="<?php echo get_permalink( $loop->post->ID ) ?>" title="<?php the_title_attribute(); ?>">
								...
								<div class="top-grid-img">  
									<?php if ( has_post_thumbnail( $loop->post->ID ) ) { echo get_the_post_thumbnail( $loop->post->ID, 'maxstore-home-top' ); }
									else { echo '<img src="' . woocommerce_placeholder_img_src() . '" alt="Placeholder" width="300px" height="300px" />'; } ?>
								</div>
								<div class="top-grid-heading">
									<h2><?php the_title(); ?></h2>
